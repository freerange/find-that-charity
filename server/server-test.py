...
for e_v in POTENTIAL_ENV_VARS:
    if os.environ.get(e_v):
        app.config["mongo"] = pymongo.MongoClient(os.environ.get(e_v))
        app.config["mongo_db"] = 'charitysearch'
        app.config["mongo_collection"] = 'organisation'
        break
...
def search_return(query, term, orgtype=None):
    ...
    """
    res = app.config["mongo"][app.config["mongo_db"]][app.config["mongo_collection"]].find(
